...
import com.jishou.util.PageController;
...
//	}
	...
	public displayDoctor() {
		setIconImage(Toolkit.getDefaultToolkit().getImage(displayDoctor.class.getResource("/Source/2.png")));
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);// 设置窗体关闭按钮
		setBounds(100, 100, 610, 436);// 设置窗体位置与大小
		...
		scrollPane.setBounds(27, 80, 529, 210);// 设置大小与位置
		contentPane.add(scrollPane);// 将滚动面板加入到内容面板中

		// 使用动态数组数据（列标题与行数据）
		titles = new Vector<String>();// 定义动态数组表示表格标题
		Collections.addAll(titles, "编号", "姓名","科室", "医院","类别");
...
//		使用静态数据创建DefaultTableModel数据模型
		...
		lblNewLabel.setIcon(new ImageIcon(displayDoctor.class.getResource("/Source/\u56FE\u7247.jpg")));
		lblNewLabel.setBounds(0, 0, 584, 387);
